...
class AIProcessor:
    ...
    def __init__(self, config: Config, logger: Logger):
        ...
        self._analysis_lock = Lock()  # For thread-safe operations
    ...
    def analyze_document_multipage(self, question: str, image_paths: List[str]) -> ProcessingResult:
        ...
        try:
            ...
            if not relevant_pages:
                return ProcessingResult(
                    success=True,
                    data="No relevant content found in the document for your question. The document may not contain information related to your query.",
                    metadata={"pages_analyzed": len(page_results), "relevant_pages": 0}
                )
...
class DocumentProcessor:
    ...
    def answer_question(self, question: str, query_type: QueryType = None) -> ProcessingResult:
        ...
        if query_type == QueryType.DOCUMENT:
            ...
            # Try web search first, fallback to general AI if it fails
            web_result = self.ai_processor.perform_web_search(question)
            if web_result.success and web_result.data and web_result.data.strip():
                return web_result
            else:
                self.logger.warning("Web search failed, using general AI knowledge")
                return self.ai_processor.answer_general_question(question)
