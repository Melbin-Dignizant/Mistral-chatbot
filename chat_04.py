...
#     def _get_combined_synthesis_prompt(self) -> str:
#         """Return prompt for synthesizing document and web search results."""
#         return '''You are an expert research assistant. Your task is to synthesize information from two sources to answer a user's question:
#                 1. **Internal Documents**: Content extracted from one or more private documents provided to you. This is the primary source of truth.
#                 2. **Web Search Results**: Information gathered from a real-time web search to supplement the document content.

#                 **Your Goal**: Create a single, comprehensive, and well-structured answer.

#                 **Instructions**:
#                 1.  **Prioritize Document Information**: Start with the answer found in the internal documents.
#                 2.  **Integrate Web Information**: Use the web search results to supplement, expand upon, or provide context (e.g., more recent data, definitions, background).
#                 3.  **Clear Attribution**: You MUST clearly label the source of information. Use markers like `[From Document(s)]` and `[From Web Search]`. If information is corroborated, you can state that.
#                 4.  **Handle Missing Information**:
#                     - If documents have no relevant information, state that clearly and provide the answer based only on the web search.
#                     - If the web search provides no relevant information, state that and use only the document information.
#                     - If neither source has the answer, state that you could not find the information.
#                 5.  **Structure**: Organize your answer logically. Use headings or bullet points if it improves clarity.

#                 **Output Format Example**:
#                 **Overall Summary**: <A brief, one-sentence summary of the final answer.>

#                 **From the Document(s)**:
#                 <Detailed answer based on the provided document content, including page/document references.>

#                 **Additional Information from Web Search**:
#                 <Supplementary information from the web search.>

#                 **Conclusion**: <A concluding thought or summary.>
#                 '''

#     def analyze_multi_document_query(self, question: str, state: MultiDocumentState,
#                                      conversation_context: str = "") -> ProcessingResult:
#         """
#         Analyzes a query across multiple documents and simultaneously performs a web search,
...
#             return ProcessingResult(
#                 success=False,
#                 error="No documents loaded for analysis. Please load a document first."
#             )

#         self.logger.processing(
#             f"Starting combined analysis for '{question[:50]}...' across {len(loaded_docs)} document(s) and the web."
#         )

#         doc_analysis_result = None
#         web_search_proc_result = None

#         # Run document analysis and web search in parallel
#         with concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="QueryRunner") as executor:
#             future_docs = executor.submit(self._perform_document_analysis_for_synthesis, question, state)
#             future_web = executor.submit(self.perform_web_search, question)

#             # Collect results
#             doc_analysis_result = future_docs.result()
#             web_search_proc_result = future_web.result()

#         doc_success = doc_analysis_result.get("success", False)
#         doc_data = doc_analysis_result.get("data")
#         doc_metadata = doc_analysis_result.get("metadata", {})

#         web_success = web_search_proc_result.success
#         web_data = web_search_proc_result.data if web_success else web_search_proc_result.error

#         # If both failed, fallback to general knowledge
#         if not doc_success and not web_success:
#             self.logger.error("Both document analysis and web search failed. Falling back to general knowledge.")
#             response = self.answer_general_question(question, from_doc_fallback=True)
#             return response

#         self.logger.processing("Synthesizing results from documents and web search...")

#         # Synthesize the final answer from the collected data
#         final_answer = self._synthesize_combined_results(
#             question=question,
#             document_content=doc_data if doc_success else None,
#             web_content=web_data if web_success else None,
#             conversation_context=conversation_context
#         )

#         # Combine metadata for a comprehensive report
#         final_metadata = {
#             "document_analysis": doc_metadata,
#             "web_search_successful": web_success,
...
#     def _perform_document_analysis_for_synthesis(self, question: str, state: MultiDocumentState) -> Dict[str, Any]:
#         """
#         Performs the document analysis part of a combined query, returning a dictionary with results.
#         """
#         try:
#             loaded_docs = state.get_loaded_documents()
#             self.logger.processing(f"Analyzing query across {len(loaded_docs)} documents...")
...
#                 for doc in loaded_docs:
#                     for i, image_path in enumerate(doc.image_paths):
#                         future = executor.submit(self.analyze_single_page, question, image_path, i + 1, doc.document_id)
#                         future_to_page[future] = (doc.document_id, i + 1)
...
#             if not relevant_pages:
#                 self.logger.info("No relevant content found in documents.")
#                 return {
#                     "success": False,
#                     "data": "No relevant information was found in the provided document(s).",
...
#             # Synthesize document results into a coherent summary
#             document_synthesis = self._synthesize_multi_document_results(
#                 question, pages_by_doc, state, ""
#             )

#             referenced_docs = list(pages_by_doc.keys())
#             return {
#                 "success": True,
#                 "data": document_synthesis,
#                 "metadata": {
#                     "documents_searched": len(loaded_docs),
#                     "relevant_pages": len(relevant_pages),
#                     "referenced_documents": referenced_docs,
...
#             }
#         except Exception as e:
#             self.logger.error(f"Internal document analysis failed: {str(e)}")
#             return {"success": False, "data": f"Error during document analysis: {e}", "metadata": {}}
...
#     def _synthesize_combined_results(self, question: str, document_content: Optional[str],
#                                      web_content: Optional[str], conversation_context: str = "") -> str:
#         """Synthesizes results from document analysis and web search into a final answer."""
#         try:
#             if not document_content and not web_content:
#                 return "I could not find any information in the documents or from a web search to answer your question."

#             # Prepare the input for the final synthesis model
#             synthesis_input_parts = [f"User Question: {question}"]
#             if conversation_context:
#                 synthesis_input_parts.append(f"Previous Conversation Context: {conversation_context}")

#             synthesis_input_parts.append("\n--- Information from Internal Documents ---")
#             synthesis_input_parts.append(document_content or "No relevant information was found in the internal documents.")

#             synthesis_input_parts.append("\n--- Information from Web Search ---")
#             synthesis_input_parts.append(web_content or "The web search did not yield any relevant results.")
            
#             synthesis_input = "\n".join(synthesis_input_parts)

#             messages = [
#                 {"role": "system", "content": self._get_combined_synthesis_prompt()},
...
#         except Exception as e:
#             self.logger.error(f"Final synthesis of combined results failed: {e}")
#             fallback_answer = "I found information from multiple sources but encountered an error while combining them.\n\n"
#             if document_content:
#                 fallback_answer += f"**From Documents:**\n{document_content}\n\n"
#             if web_content:
#                 fallback_answer += f"**From Web Search:**\n{web_content}"
...
#             with concurrent.futures.ThreadPoolExecutor(max_workers=self.config.max_concurrent_pages) as executor:
#                 future_to_page = {
#                     executor.submit(self.analyze_single_page, question, image_path, i + 1, "single_doc"): i + 1
#                     for i, image_path in enumerate(image_paths)
#                 }
...
# Question Types:
# - Regular question       : Automatically analyze documents and perform a web search for context.
# - Questions with words like 'latest', 'current' will trigger a web-only search.
# - Questions with words like 'compare', 'between' will analyze multiple documents and the web.
#         """
#         print(help_text)
#         return True
